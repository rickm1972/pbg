...
export function Gate1SourcesReviewedPanel({ evidence }: Props) {
  ...
  const { coverage, sections, missingUrlNotes, allRows, providedIntake, agent1SourceNotes } = model
  ...
  return (
    <section className="mt-4 space-y-4 rounded-xl border-2 border-sky-200 bg-sky-50/40 p-4">
      ...
      {providedIntake.length > 0 ? (
        <div className="rounded-lg border border-indigo-200 bg-indigo-50/60 p-3">
          <h5 className="text-xs font-semibold uppercase tracking-wide text-indigo-900">
            Provided source intake
          </h5>
          <p className="mt-1 text-xs text-indigo-950">
            URLs supplied before Agent 1 — validation results and whether each was used as primary evidence.
          </p>
          {agent1SourceNotes ? (
            <p className="mt-2 text-xs text-indigo-900">
              <span className="font-semibold">Agent 1 notes:</span> {agent1SourceNotes}
            </p>
          ) : null}
          <ul className="mt-2 space-y-2">
            {providedIntake.map((row) => (
              <li key={row.url} className="rounded-md border border-indigo-100 bg-white p-2 text-xs">
                <p className="font-semibold text-ink-900">{row.intendedRole}</p>
                <a
                  href={row.url}
                  target="_blank"
                  rel="noreferrer"
                  className="mt-1 inline-block font-semibold text-indigo-800 underline"
                >
                  Open provided URL
                </a>
                <p className="mt-1 text-slate-600">
                  ...
                  {row.validationPassed === true
                    ? 'passed'
                    : row.validationPassed === false
                      ? 'failed'
                      : row.fetchOk
                        ? 'fetched'
                        : 'not validated'}
                  {row.usedAsPrimaryEvidence ? ' · used as primary evidence' : ''}
                </p>
                {row.failureReason ? (
                  <p className="mt-1 text-red-900">{row.failureReason}</p>
                ) : null}
                {row.searchDiscoveredAlternateUrl ? (
                  <p className="mt-1 text-amber-950">
                    Search discovered alternate (supplemental): {row.searchDiscoveredAlternateUrl}
                  </p>
                ) : null}
                {row.fieldsSupported.length > 0 ? (
                  <p className="mt-1 text-slate-600">
                    Fields supported: {row.fieldsSupported.join(' · ')}
                  </p>
                ) : null}
              </li>
            ))}
