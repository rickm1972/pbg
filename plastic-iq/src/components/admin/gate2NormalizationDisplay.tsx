...
import { whyFieldsFromScoringInput } from '../../lib/whyThisScoreApi'
import { WhyThisScoreAdmin } from '../WhyThisScoreAdmin'

export { whyFieldsFromScoringInput }
...
export function ProductDescriptionSection({ inputs }: { inputs: NormalizationInputs }) {
  const legacyFailed = inputs.status === 'description_generation_failed'
  const cosmeticWarning =
    inputs.product_description_status === 'warning' ||
    inputs.product_description_status === 'fallback_generated' ||
    legacyFailed
  const text = inputs.product_description?.trim()

  if (cosmeticWarning && !text) {
    return (
      <Gate2Section title="Product description">
        <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
          <span className="font-semibold">Non-score copy warning.</span>{' '}
          {inputs.product_description_warnings?.[0] ??
            (inputs.flagged_missing_fields?.length
              ? `Cosmetic copy issue: ${inputs.flagged_missing_fields.join(', ')}`
              : 'Description copy was not generated; scoring inputs are unaffected.')}
        </p>
      </Gate2Section>
    )
  }
  ...
  const wordCount =
    inputs.description_word_count ?? text.split(/\s+/).filter(Boolean).length
    ...
    <Gate2Section title="Product description">
      {cosmeticWarning ? (
        <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
          <span className="font-semibold">Non-score copy warning.</span>{' '}
          {inputs.product_description_warnings?.[0] ??
            'Description copy is outside the configured word-count range; scoring inputs are unaffected.'}
        </p>
      ) : null}
      <p className={`text-xs text-slate-500${cosmeticWarning ? ' mt-2' : ''}`}>
        Template-driven copy ({wordCount} words).
        {genVersion ? ` Generator: ${genVersion}.` : ''}
        {inputs.product_description_status ? ` Status: ${inputs.product_description_status}.` : ''}
      </p>
...
export function getNormalizationWarnings(
  ...
  if (
    inputs.product_description_status === 'warning' ||
    inputs.product_description_status === 'fallback_generated' ||
    inputs.status === 'description_generation_failed'
  ) {
    warnings.push(
      inputs.product_description_warnings?.[0] ??
        'Product description copy has a non-score validation warning; scoring inputs were still produced.',
    )
  }
  return warnings
}
    ...
    <>
      ...
      {whyFields ? (
        <Gate2Section title="Why this score (structured)">
          <WhyThisScoreAdmin fields={whyFields} className="mt-2 border-0 p-0 shadow-none" />
        </Gate2Section>
      ) : null}
