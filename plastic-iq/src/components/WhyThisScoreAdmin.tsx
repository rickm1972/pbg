/** Admin-only Why This Score panel — reads raw Gate 2 fields (not public APR renderer). */
...
export function WhyThisScoreAdmin({ fields, className = '' }: Props) {
  return (
    <div
              ...
              <dd className="mt-1.5">
                {visible.length > 0 ? (
                  <ul className="list-none space-y-2 pl-0 leading-relaxed">
                    {visible.map((option) => {
                      const graphiteInternal = /^Graphite core$/i.test(option)
                      return (
                        <li key={option}>
                          <div>{option}</div>
                          {graphiteInternal ? (
                            <p className="mt-1 text-xs leading-snug text-slate-500">
                              Internal bonded core — not a food-contact surface.
                            </p>
                          ) : null}
                        </li>
                      )
