#!/usr/bin/env node
/**
 * Phase 3.5: apply canonical_mappings to T-Fal pending_review evidence (no Agent 1 re-run).
 * Usage: node scripts/phase35-backfill-tfal-canonical.mjs [--apply]
 */
...
const mappings = applyCanonicalMappings(structured, row.sources ?? [], { facts: row.facts ?? [] })

console.log('T-Fal canonical_mappings (dry-run=%s):', !apply)
console.log(JSON.stringify(mappings, null, 2))

if (!apply) {
  console.log('\nPass --apply to persist to product_evidence.')
  process.exit(0)
}
